...
### 1. Lifecycle Configuration Warning
**Error:**
```
...
**Cause:** AWS provider v5.90+ requires each lifecycle `rule` to have either `filter` block or `prefix`, but none was specified.
...
### 2. S3 Bucket Policy - MalformedPolicy
**Error:**
```
Error: putting S3 Bucket Policy: MalformedPolicy: Unknown field Resources
```

**Cause:** Policy JSON uses `"Resources"` (plural) instead of required `"Resource"` (singular).
...
resource "aws_s3_bucket_policy" "public_read" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Principal = "*"
      Action    = "s3:GetObject"
      Resource  = "arn:aws:s3:::${aws_s3_bucket.project_bucket.id}/*"  # Note: "Resource" (singular)
    }]
  })
}
```

### 3. S3 Bucket Policy - AccessDenied (Block Public Policy)
**Error:**
```
AccessDenied: User is not authorized to perform: s3:PutBucketPolicy because public policies are prevented by BlockPublicPolicy
```

**Cause:** S3 Block Public Access setting `BlockPublicPolicy` prevents public-read policies.

**Solutions (choose one):**

**Option A: Comment out public policy (Recommended)**
```hcl
/*
resource "aws_s3_bucket_policy" "public_read" {
...
**Option B: Disable Block Public Policy**
```hcl
resource "aws_s3_bucket_public_access_block" "project_bucket" {
  bucket = aws_s3_bucket.project_bucket.id

  block_public_acls       = false
  block_public_policy     = false  # Key setting
...
1. Comment out the public policy with `/* */` (fastest fix)
2. Run `terraform plan` to verify
3. Run `terraform apply` to complete deployment

All errors resolved with these changes!
